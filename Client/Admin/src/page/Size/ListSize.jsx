...
const ListSize = () => {
  const { sizes, setSizes } = useContext(ThemeApp);
  const [search, setSearch] = useState("");
  const UrlFetchApiGet = `http://localhost:2004/api/size/get-all-sizes`;
  const UrlFetchApiDelete = `http://localhost:2004/api/size/delete-size`;

  const FetchApiSizes = async () => {
    try {
      const result = await axios.get(`${UrlFetchApiGet}`);
      const category = result.data;
      setSizes(category.data);
    } catch (err) {
        ...
        <table className="table-fixed w-full">
          <thead className="uppercase bg-red-500">
            <tr>
              ...
              <th className="w-[60%] py-2">Tên kích cỡ</th>
                    ...
                    <td>
                      <div className="flex items-center justify-center gap-3">
                        <button className="bg-green-200 rounded p-2 cursor-pointer text-green-600">
                          <Link to={`/size/update-size/${item._id}`}>
                            <FaRegEdit size={16} />
                          </Link>
                        </button>
                        <button
