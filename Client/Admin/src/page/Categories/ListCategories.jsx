...
const ListCategories = () => {
  const { categories, setCategories } = useContext(ThemeApp);
  const [search, setSearch] = useState("");
  const UrlFetchApiGet = `http://localhost:2004/api/category/get-all-category`;
  const UrlFetchApiDelete = `http://localhost:2004/api/category/delete-category`;
        ...
        <table className="table-fixed w-full">
          <thead className="uppercase bg-red-500">
            <tr>
              ...
              <th className="w-[60%] py-2">Tên thể loại</th>
                    ...
                    <td>
                      <div className="flex items-center justify-center gap-3">
                        <button className="bg-green-200 rounded p-2 cursor-pointer text-green-600">
                          <Link to={`/category/update-category/${item._id}`}>
                            <FaRegEdit size={16} />
                          </Link>
                        </button>
                        <button
